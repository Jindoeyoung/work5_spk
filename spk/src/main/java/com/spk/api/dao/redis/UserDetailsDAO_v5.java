...
//import com.spk.api.entity.usergrids.UserGrids;

@Repository
public class UserDetailsDAO_v5 {

    private static final String USER_DETAILS_KEY = "USER_DETAILS_V5:${USER_ID}";
...
//        this.stringRedisTemplate = stringRedisTemplate;
    ...
    public UserDetails getKeyValue(String userid) throws IOException {
        String key = StringSubstitutor.replace(
            USER_DETAILS_KEY, ImmutableMap.of("USER_ID", userid)
        );
        ...
        return messagePackObjectMapper.readValue(message, UserDetails.class);
    }

    public void setKeyValue(UserDetails userApi) throws JsonProcessingException {
        String key = StringSubstitutor.replace(
            USER_DETAILS_KEY, ImmutableMap.of("USER_ID", userApi.getUser_id())
        );
    ...
    public List<String> getAllKeyValues() {
        String key = StringSubstitutor.replace(USER_DETAILS_KEY, ImmutableMap.of(
            "USER_ID", "*"
        ));
        ...
        while (cursor.hasNext()) {
            String user = StringUtils.replace(new String(cursor.next()), "USER_DETAILS_V5:", "");
